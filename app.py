...
<head>
    ...
    <script>
        function copyTerminalData() {
            ...
            let textToCopy = "SEMI-PRO TERMINAL EXPORT:\\n";
            cards.forEach(card => {
                let ticker = card.querySelector('strong').innerText;
                let priceInfo = card.querySelector('.price-line').innerText;
                let kgv = card.querySelector('.kgv-tag') ? card.querySelector('.kgv-tag').innerText : "";
                let status = card.querySelector('.status-text').innerText;
                let rsi = card.querySelector('.rsi-badge').innerText;
                textToCopy += `- ${ticker}: ${priceInfo} | ${kgv} | ${status} | ${rsi}\\n`;
            });
            navigator.clipboard.writeText(textToCopy).then(() => {
                alert("Daten inklusive KGV kopiert!");
            });
        }
    </script>
</head>
<body>
    <div class="container">
        <h2 onclick="copyTerminalData()" style="color:#ffd700; cursor:pointer; letter-spacing: 1px;">PRO TERMINAL 🚀</h2>
        <div class="input-area">
            <form action="/stack" method="POST">
                <input type="text" name="symbol" placeholder="Ticker/WKN..." required autofocus>
        ...
        <div class="card">
            <div>
                <strong style="font-size:1.3em;">{{ s.ticker }}</strong>
                <span class="kgv-tag">KGV: {{ s.kgv }}</span><br>
                <div class="price-line" style="color:#fff; font-weight: bold; margin-top: 3px;">
                    {{ s.price }}€ <span style="color:{{ s.change_color }}; font-size: 0.85em;">({{ s.change }})</span>
                </div>
                <div class="status-text" style="color:{{ '#00ff88' if s.status == 'OVER' else '#ff4d4d' }}">{{ s.status }} SMA38</div>
                <small style="color:#555;">SMA38: {{ s.sma38 }}</small>
            </div>
